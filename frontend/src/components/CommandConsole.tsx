  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-lg p-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-sm font-bold text-cyan-400 font-mono uppercase tracking-wide">
            System Entanglement Matrix
          </h3>
          <p className="text-xs text-gray-500 mt-0.5">Family dynamics entropy analysis</p>
        </div>
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded ${stressBg}`}>
          <Activity className={`w-4 h-4 ${stressColor}`} />
          <span className={`text-sm font-bold ${stressColor}`}>{stressLevel} Stress</span>
        </div>
      </div>
        ...
        {nodes.map((node) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div className={`w-2 h-2 rounded-full ${node.role === 'Primary' ? 'bg-cyan-400' : 'bg-gray-500'}`} />
              <div>
  ...
  return (
    <section className="bg-gray-950 border-t border-cyan-900/50 pt-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold text-cyan-400 font-mono">Command Console</h2>
          <p className="text-xs text-gray-500">System dynamics - Tactical protocols - Market resonance</p>
        </div>
      </div>
